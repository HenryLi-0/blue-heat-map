...
Here's what we log! `(delta timestamp, x, y, z)`, and that's all! After a set amount of sessions, the system should automatically compress `.db` files into a zip! Running this on a set of ten `.db` files for 3600 logs each (representative of one log per second for an hour), we get `1433600 bytes`. That's for data looking like: `(1155 843086 369847 126799)` for each log! Afterwards, after compression of these ten log files, we get `774144 bytes`, a 46% reduction! (coincidentally, it's actually perfectly 46%)
